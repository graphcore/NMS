...
public:
  NmsOp(const popart::OperatorIdentifier &_opid, const float threshold,
        ...
        bool useGather, bool inPlace, const popart::Op::Settings &settings_)
      : popart::Op(_opid, settings_), threshold_{threshold},
        scoreThreshold_{scoreThreshold}, numDetections_{numDetections},
        sigma_{sigma}, useGather_{useGather}, inPlace_{inPlace} {}
  ...
  virtual void setup() {
    ...
    outInfo(3).set(classesInfo.dataType(), {batchSize, numDetections_});
  ...
  void appendAttributes(popart::OpSerialiserBase &os) const override {
    ...
    os.appendAttribute("inPlace", getInPlace());
  }
  ...
  bool getInPlace() const { return inPlace_; }
  ...
  void grow(poplar::program::Sequence &prog) const final {
    ...
    std::tie(indicesAns, scoresAns, boxesAns, classesAns, lengthsAns) =
        nms(graph(), prog, scores, boxes, classes, threshold, numDetections,
            scoreThreshold, sigma, useGather, inPlace);
    // for pytorch only ...
